...
#define main xqc_demo_wifi_monitor_standalone_main
#include "../../ebpf-wifi-insider/monitor.c"
#undef main

#define XQC_DEMO_WIFI_MONITOR_MAX_IFACES 2
#define XQC_DEMO_WIFI_MONITOR_MIN_SAMPLES 5
#define XQC_DEMO_WIFI_MONITOR_LOG_INTERVAL_US 1000000ULL
#define XQC_DEMO_WIFI_MONITOR_KEEP_STATUS INT_MIN
#define XQC_DEMO_WIFI_MONITOR_START_WAIT_US 5000000ULL

typedef struct xqc_demo_wifi_iface_state_s {
    char                            ifname[64];
    unsigned int                    ifindex;
    xqc_demo_wifi_state_snapshot_t  snapshot;
    double                          ewma_long_txop_interval;
    uint64_t                        last_logged_ts_us;
} xqc_demo_wifi_iface_state_t;

struct xqc_demo_wifi_monitor_s {
    pthread_t                       thread;
    pthread_mutex_t                 lock;
    xqc_demo_wifi_monitor_config_t  config;
    xqc_demo_wifi_iface_state_t     ifaces[XQC_DEMO_WIFI_MONITOR_MAX_IFACES];
    int                             iface_count;
    int                             running;
    int                             stop_requested;
    int                             start_rc;
...
static xqc_demo_wifi_monitor_t *g_xqc_demo_wifi_monitor = NULL;

static double
xqc_demo_wifi_clip_prob(double value)
{
    if (value < 1e-9) {
        return 1e-9;
    }
...
static xqc_demo_wifi_iface_state_t *
xqc_demo_wifi_find_iface_locked(xqc_demo_wifi_monitor_t *monitor, unsigned int ifindex)
{
    int i;
...
static void
xqc_demo_wifi_monitor_update_status(xqc_demo_wifi_monitor_t *monitor,
    int start_rc, int running, int start_done)
{
    ...
    if (start_rc != XQC_DEMO_WIFI_MONITOR_KEEP_STATUS) {
        monitor->start_rc = start_rc;
    }
    if (running != XQC_DEMO_WIFI_MONITOR_KEEP_STATUS) {
        monitor->running = running;
    }
    if (start_done != XQC_DEMO_WIFI_MONITOR_KEEP_STATUS) {
...
static void
xqc_demo_wifi_copy_cstr(char *dst, size_t dst_len, const char *src)
{
...
static int
xqc_demo_wifi_join_path(char *dst, size_t dst_len, const char *dir, const char *name)
{
    int ret;
...
static void
xqc_demo_wifi_emit_state_locked(xqc_demo_wifi_monitor_t *monitor,
    xqc_demo_wifi_iface_state_t *iface_state, uint64_t ts_us, int force)
{
    xqc_demo_wifi_state_snapshot_t snapshot;
    ...
    if (!force && iface_state->last_logged_ts_us != 0
        && ts_us < iface_state->last_logged_ts_us + XQC_DEMO_WIFI_MONITOR_LOG_INTERVAL_US)
    {
        return;
    }
...
static void
xqc_demo_wifi_update_iface_state(xqc_demo_wifi_monitor_t *monitor,
    const struct event_wifi_pkt *event, long long offset)
{
    ...
    xqc_demo_wifi_iface_state_t *iface_state;
    ...
    if (monitor == NULL || event == NULL || event->txop_interval_us == 0) {
        return;
    }

    pthread_mutex_lock(&monitor->lock);
    iface_state = xqc_demo_wifi_find_iface_locked(monitor, event->tx_ifindex);
    if (iface_state == NULL) {
        pthread_mutex_unlock(&monitor->lock);
        return;
    }
    ...
    pi_pred = xqc_demo_wifi_clip_prob(pi_pred);
    ...
    if (iface_state->snapshot.sample_count < XQC_DEMO_WIFI_MONITOR_MIN_SAMPLES) {
        iface_state->snapshot.state = XQC_DEMO_WIFI_PATH_STATE_UNKNOWN;

    } else if (old_state == XQC_DEMO_WIFI_PATH_STATE_UNKNOWN) {
        iface_state->snapshot.state = iface_state->snapshot.pi_degraded >= theta_high
            ? XQC_DEMO_WIFI_PATH_STATE_DEGRADED_CSMA
            : XQC_DEMO_WIFI_PATH_STATE_REGULAR;

    } else if (old_state == XQC_DEMO_WIFI_PATH_STATE_REGULAR
        && iface_state->snapshot.pi_degraded >= theta_high)
    {
        iface_state->snapshot.state = XQC_DEMO_WIFI_PATH_STATE_DEGRADED_CSMA;

    } else if (old_state == XQC_DEMO_WIFI_PATH_STATE_DEGRADED_CSMA
        && iface_state->snapshot.pi_degraded <= theta_low)
    {
        iface_state->snapshot.state = XQC_DEMO_WIFI_PATH_STATE_REGULAR;
    }

    xqc_demo_wifi_emit_state_locked(monitor, iface_state, ts_us,
        iface_state->snapshot.state != old_state);
    pthread_mutex_unlock(&monitor->lock);
}

static int
xqc_demo_embedded_handle_wifi_pkt(void *ctx, void *data, size_t data_sz)
{
    int ret;

    ret = handle_wifi_pkt(ctx, data, data_sz);
    if (g_xqc_demo_wifi_monitor != NULL && data_sz >= sizeof(struct event_wifi_pkt)) {
        xqc_demo_wifi_update_iface_state(g_xqc_demo_wifi_monitor,
...
static int
xqc_demo_embedded_handle_wifi_rx(void *ctx, void *data, size_t data_sz)
{
    return handle_wifi_rx(ctx, data, data_sz);
}

static int
xqc_demo_embedded_handle_mac_tx(void *ctx, void *data, size_t data_sz)
{
    return handle_mac_tx(ctx, data, data_sz);
}

static int
xqc_demo_embedded_handle_tcp_retrans(void *ctx, void *data, size_t data_sz)
{
    return handle_tcp_retrans(ctx, data, data_sz);
}

static int
xqc_demo_embedded_handle_tcp_loss(void *ctx, void *data, size_t data_sz)
{
    return handle_tcp_loss(ctx, data, data_sz);
}

static int
xqc_demo_embedded_handle_tcp_recovery(void *ctx, void *data, size_t data_sz)
{
    return handle_tcp_recovery(ctx, data, data_sz);
}

static int
xqc_demo_embedded_handle_drop(void *ctx, void *data, size_t data_sz)
{
    return handle_drop(ctx, data, data_sz);
}

static void
xqc_demo_wifi_monitor_set_error(xqc_demo_wifi_monitor_t *monitor, const char *message)
{
...
static int
xqc_demo_wifi_monitor_open_outputs(xqc_demo_wifi_monitor_t *monitor)
{
    char path_buf[512];
    int ret;
...
#define XQC_DEMO_OPEN_FILE(fp, name, header) do { \
    ret = xqc_demo_wifi_join_path(path_buf, sizeof(path_buf), output_dir, name); \
    if (ret == 0) { \
        fp = fopen(path_buf, "w"); \
        if (fp != NULL) { \
            fprintf(fp, "%s\n", header); \
    ...
    XQC_DEMO_OPEN_FILE(fp_tcp_loss, "tcp_loss.csv",
        "ts,src_ip,src_port,dst_ip,dst_port,cwnd,ssthresh,icsk_backoff,srtt");
    XQC_DEMO_OPEN_FILE(fp_tcp_recovery, "tcp_recovery.csv",
        "ts,src_ip,src_port,dst_ip,dst_port,cwnd,ssthresh");
    XQC_DEMO_OPEN_FILE(fp_drop, "drop_trace.csv",
...

    {
        ...
        XQC_DEMO_OPEN_FILE(fp_mgmt, mgmt_name, "ts,type,event,rssi,snr,raw_msg");
    }

    if (g_ifname2[0] != '\0') {
        char mgmt_name[128];
        snprintf(mgmt_name, sizeof(mgmt_name), "mgmt_trace_%s.csv", g_ifname2);
        XQC_DEMO_OPEN_FILE(fp_mgmt2, mgmt_name, "ts,type,event,rssi,snr,raw_msg");
    }

#undef XQC_DEMO_OPEN_FILE
    (void) monitor;
    return 0;
}

static void
xqc_demo_wifi_monitor_write_meta(xqc_demo_wifi_monitor_t *monitor)
{
    FILE *fp;
...
static void
xqc_demo_wifi_monitor_merge_mgmt(xqc_demo_wifi_monitor_t *monitor)
{
    FILE *dst;
    int i;
    int ret;
...
static void *
xqc_demo_wifi_monitor_thread_main(void *arg)
{
    xqc_demo_wifi_monitor_t *monitor = arg;
    ...
    g_xqc_demo_wifi_monitor = monitor;
    exiting = false;
    g_rb = NULL;
    xqc_demo_wifi_copy_cstr(output_dir, sizeof(output_dir), monitor->output_dir);
    xqc_demo_wifi_copy_cstr(g_ifname, sizeof(g_ifname),
        monitor->iface_count > 0 ? monitor->ifaces[0].ifname : "");
    xqc_demo_wifi_copy_cstr(g_ifname2, sizeof(g_ifname2),
        monitor->iface_count > 1 ? monitor->ifaces[1].ifname : "");

    if (parse_json_config(monitor->config_path) != 0) {
        xqc_demo_wifi_monitor_set_error(monitor, "parse monitor config failed");
        xqc_demo_wifi_monitor_update_status(monitor, -1, 0, 1);
        xqc_demo_wifi_monitor_write_meta(monitor);
        g_xqc_demo_wifi_monitor = NULL;
        return NULL;
    }

    if (mkdir(output_dir, 0755) != 0 && errno != EEXIST) {
        xqc_demo_wifi_monitor_set_error(monitor, "create output dir failed");
        xqc_demo_wifi_monitor_update_status(monitor, -1, 0, 1);
        xqc_demo_wifi_monitor_write_meta(monitor);
        g_xqc_demo_wifi_monitor = NULL;
        return NULL;
    ...
    xqc_demo_wifi_monitor_open_outputs(monitor);

    skel = monitor_bpf__open();
    if (skel == NULL) {
        xqc_demo_wifi_monitor_set_error(monitor, "open BPF skeleton failed");
        xqc_demo_wifi_monitor_update_status(monitor, -1, 0, 1);
        goto cleanup;
    }
    ...
    if (err) {
        xqc_demo_wifi_monitor_set_error(monitor, "load BPF skeleton failed");
        xqc_demo_wifi_monitor_update_status(monitor, err, 0, 1);
        goto cleanup;
    }

    err = monitor_bpf__attach(skel);
    if (err) {
        xqc_demo_wifi_monitor_set_error(monitor, "attach BPF skeleton failed");
        xqc_demo_wifi_monitor_update_status(monitor, err, 0, 1);
        goto cleanup;
    }
    ...
    rb = ring_buffer__new(bpf_map__fd(skel->maps.rb_wifi_pkt),
        xqc_demo_embedded_handle_wifi_pkt, &offset, NULL);
    g_rb = rb;
    if (rb == NULL) {
        xqc_demo_wifi_monitor_set_error(monitor, "create ring buffer failed");
        xqc_demo_wifi_monitor_update_status(monitor, -1, 0, 1);
        goto cleanup;
    }

    ring_buffer__add(rb, bpf_map__fd(skel->maps.rb_wifi_rx),
        xqc_demo_embedded_handle_wifi_rx, &offset);
    ring_buffer__add(rb, bpf_map__fd(skel->maps.rb_mac_tx),
        xqc_demo_embedded_handle_mac_tx, &offset);
    ring_buffer__add(rb, bpf_map__fd(skel->maps.rb_tcp_retrans),
        xqc_demo_embedded_handle_tcp_retrans, &offset);
    ring_buffer__add(rb, bpf_map__fd(skel->maps.rb_tcp_loss),
        xqc_demo_embedded_handle_tcp_loss, &offset);
    ring_buffer__add(rb, bpf_map__fd(skel->maps.rb_tcp_recovery),
        xqc_demo_embedded_handle_tcp_recovery, &offset);
    ring_buffer__add(rb, bpf_map__fd(skel->maps.rb_drop),
        xqc_demo_embedded_handle_drop, &offset);
    ...
    xqc_demo_wifi_monitor_update_status(monitor, 0, 1, 1);
    xqc_demo_wifi_monitor_write_meta(monitor);
    ...
    while (!monitor->stop_requested && !exiting) {
        ...
        if (err < 0) {
            xqc_demo_wifi_monitor_set_error(monitor, "poll ring buffer failed");
            xqc_demo_wifi_monitor_update_status(monitor, err,
                XQC_DEMO_WIFI_MONITOR_KEEP_STATUS,
                XQC_DEMO_WIFI_MONITOR_KEEP_STATUS);
            break;
        }
    }
...
cleanup:
    exiting = true;
    xqc_demo_wifi_monitor_update_status(monitor,
        XQC_DEMO_WIFI_MONITOR_KEEP_STATUS, 0, 1);
    if (mgmt1_started) {
    ...
    close_all_files();
    xqc_demo_wifi_monitor_merge_mgmt(monitor);
    xqc_demo_wifi_monitor_write_meta(monitor);
    g_xqc_demo_wifi_monitor = NULL;
    return NULL;
}
...
{
    xqc_demo_wifi_monitor_t *monitor;
    ...
    *monitor_out = NULL;
    ...
    xqc_demo_wifi_copy_cstr(monitor->output_dir, sizeof(monitor->output_dir), config->output_dir);
    xqc_demo_wifi_copy_cstr(monitor->config_path, sizeof(monitor->config_path),
        config->config_path ? config->config_path : "ebpf-wifi-insider/config.json");
    xqc_demo_wifi_join_path(monitor->meta_path, sizeof(monitor->meta_path),
        monitor->output_dir, "wifi_module_meta.json");
    xqc_demo_wifi_join_path(monitor->merged_mgmt_path, sizeof(monitor->merged_mgmt_path),
        monitor->output_dir, "mgmt_trace.csv");

    xqc_demo_wifi_copy_cstr(monitor->ifaces[0].ifname, sizeof(monitor->ifaces[0].ifname),
        config->ifname_primary);
    monitor->ifaces[0].ifindex = if_nametoindex(config->ifname_primary);
    xqc_demo_wifi_copy_cstr(monitor->ifaces[0].snapshot.ifname,
        sizeof(monitor->ifaces[0].snapshot.ifname), config->ifname_primary);
    monitor->ifaces[0].snapshot.ifindex = monitor->ifaces[0].ifindex;
    monitor->ifaces[0].snapshot.state = XQC_DEMO_WIFI_PATH_STATE_UNKNOWN;
    monitor->iface_count = 1;
    monitor->start_rc = -1;
    monitor->start_done = 0;

    if (config->ifname_secondary && config->ifname_secondary[0] != '\0') {
        xqc_demo_wifi_copy_cstr(monitor->ifaces[1].ifname, sizeof(monitor->ifaces[1].ifname),
            config->ifname_secondary);
        monitor->ifaces[1].ifindex = if_nametoindex(config->ifname_secondary);
        xqc_demo_wifi_copy_cstr(monitor->ifaces[1].snapshot.ifname,
        ...
        monitor->ifaces[1].snapshot.state = XQC_DEMO_WIFI_PATH_STATE_UNKNOWN;
        monitor->iface_count = 2;
    }

    if (pthread_create(&monitor->thread, NULL, xqc_demo_wifi_monitor_thread_main, monitor) != 0) {
        pthread_mutex_destroy(&monitor->lock);
        free(monitor);
        return -1;
    }

    while (waited_us < XQC_DEMO_WIFI_MONITOR_START_WAIT_US) {
        pthread_mutex_lock(&monitor->lock);
    ...
    if (!start_done || start_rc != 0) {
        xqc_demo_wifi_monitor_stop(monitor);
        return -1;
    }
