...
def parse_packet_buffered(kv):
    return {
        "header": {"packet_number": int(kv.get("pkt_num", 0)), "packet_type": packet_type_.get(int(kv.get("pkt_type", 6)))},
...
def parse_packets_acked(kv):
    ...
    return {
        "packet_number_space": packet_number_namespace_.get(int(kv.get("pkt_space", 3))),
...
def parse_stream_state_updated(kv):
    stream_side = "receiving" if "recv_stream" in kv else "sending"
    state_map = recv_stream_states_ if stream_side == "receiving" else send_stream_states_
    return {
        "StreamType": "bidirectional", "new": state_map[int(kv.get("new", 0))], "stream_side": stream_side
    }

def parse_frames_processed(kv):
    frame_type_int = int(kv.get("type", -1))
    if frame_type_int < 0 or frame_type_int >= len(frame_type_): return None
    ...
    # Add other frame type parsing here if needed
...
def parse_stream_data_moved(kv):
    return {
        "stream_id": int(kv.get("stream_id", 0)), "offset": int(kv.get("stream_send_offset", 0)),
        "length": int(kv.get("send_data_size", 0)), "from": kv.get("from", "unknown"), "to": kv.get("to", "unknown")
    }
...
def parse_rec_metrics_updated(kv):
    return {
        "congestion_window": int(kv.get("cwnd", 0)), "bytes_in_flight": int(kv.get("inflight", 0)),
        "pacing_rate": int(kv.get("pacing_rate", 0)), "pto_count": int(kv.get("pto_count", 0)),
        "rtt_variance": int(kv.get("ctl_rttvar", 0)), "min_rtt": int(kv.get("min_rtt", 0)),
        "latest_rtt": int(kv.get("latest_rtt", 0))
    }
...
def parse_packet_lost(kv):
    return {
        "header": { "packet_number": int(kv.get("pkt_num", -1)), "packet_type": packet_type_.get(int(kv.get("pkt_type", 6)))}
    }
...
def parse_http_frame_created(kv):
    frame_type = h3_frame_type_[int(kv.get("type", 8))]
    data = {"stream_id": kv.get("stream_id", "unknown"), "frame": {"frame_type": frame_type}}
    if frame_type == "settings":
        settings = {}
        for k in ["max_field_section_size", "qpack_max_table_capacity", "qpack_blocked_streams"]:
            if k in kv: settings[k] = int(kv[k])
        data["frame"]["settings"] = settings
    # Add parsing for other H3 frame types if needed
    return data
...
def parse_line(line):
    ...
    conn_id = kv.get("conn")
    scid = kv.get("scid")
    if not conn_id: return None, None, None

    match = re.match(r'\[(.*?)\s(.*?)\]\s\[(.*?)\]', line)
    if not match: return None, None, None
    
    time_str, time_us, event_name_raw = match.groups()
    try:
        event_time = datetime.datetime.strptime(f"{time_str} {time_us}", "%Y/%m/%d %H:%M:%S %f").timestamp() * 1000
    except ValueError:
    ...
    if "path_id" in kv: event["path"] = int(kv["path_id"])
    
    parser_func_name = "parse_" + event_name_raw
    if parser_func_name in globals():
        event_data = globals()[parser_func_name](kv)
        if event_data:
...
def endpoint_events_extraction(file_name, vantagepoint):
    ...
    for line in open(file_name, 'r', encoding='utf-8', errors='ignore'):
        ...
        if conn_id not in traces_by_conn:
            traces_by_conn[conn_id] = {
...
def main():
    ...
    # ... more validation from your script

    data = {"qlog_version": "0.4", "qlog_format": "JSON", "title": "xquic qlog", 
            "description": "this is a demo qlog json of qlog-draft-07", "traces": []}
    
    if args.slog: data["traces"].extend(endpoint_events_extraction(args.slog, "server"))
    if args.clog: data["traces"].extend(endpoint_events_extraction(args.clog, "client"))
