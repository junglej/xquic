#include "src/transport/scheduler/xqc_scheduler_copy.h"
...
static size_t
xqc_copy_scheduler_size()
{
    return 0;
}

static void
xqc_copy_scheduler_init(void *scheduler, xqc_log_t *log, xqc_scheduler_params_t *param)
{
    return;
}

static void
xqc_copy_observation_set_can_send(xqc_scheduler_observation_t *observation,
...
xqc_path_ctx_t *
xqc_copy_scheduler_get_path(void *scheduler,
    ...
    int reinject, xqc_bool_t *cc_blocked)
{
    ...
    xqc_scheduler_observation_init(&observation, "copy", conn,
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list) {
        ...
        xqc_copy_observation_set_can_send(&observation, path->path_id, path_can_send);
        if (!path_can_send) {
            goto skip_path;
        }
...
skip_path:
        xqc_log(conn->log, XQC_LOG_DEBUG,
                "|copy path|conn:%p|path_id:%ui|path_srtt:%ui|path_class:%d|"
         ...
         path_class++)
    {
        if (best_path[path_class] != NULL) {
            ...
            xqc_scheduler_notify_observer(&observation);
            xqc_log(conn->log, XQC_LOG_DEBUG, "|copy_best_path:%ui|frame_type:%s|"
                    "pn:%ui|size:%ud|reinj:%d|path_class:%d|used_mask:%ui|",
    ...
    xqc_scheduler_notify_observer(&observation);
    xqc_log(conn->log, XQC_LOG_DEBUG, "|copy_no_available_path|conn:%p|reinj:%d|", conn, reinject);
    return NULL;
}

const xqc_scheduler_callback_t xqc_copy_scheduler_cb = {
    .xqc_scheduler_size             = xqc_copy_scheduler_size,
    .xqc_scheduler_init             = xqc_copy_scheduler_init,
    .xqc_scheduler_get_path         = xqc_copy_scheduler_get_path,
};
